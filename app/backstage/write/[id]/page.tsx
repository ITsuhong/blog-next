    ...
    return (
        <div className="box-border flex flex-col h-full">
            <div className="fixed top-0 z-[999] flex items-end p-1 ml-3 pb-0 bg-white">
                <div className="font-fangyuan font-bold ">
                    {blogDetail?.tag.name}
                    {blogDetail?.title}
                </div>
                {saveStatus ? (
            ...
            <div className="flex-1 pt-[25px] bg-white">
                <MdEditor className="write" footers={[]} style={{ height: '100%', border: 'none' }} onUploadImg={onUploadImg} modelValue={text} onChange={handleOptionChange} onSave={onSave} previewTheme="mk-cute" />
            </div>
        </div>
    )
}
